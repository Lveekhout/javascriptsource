function Fraction(t, n) {
    ...
    this.toLatex = () => {
        return `\\dfrac\{${t.toLatex()}\}\{${n.toLatex()}\}`
    }
    ...
    this.eval2 = () => {
        if (Number.isInteger(t) && Number.isInteger(n)) {
            ...
            const t_ = t.eval2()
            const n_ = n.eval2()
            return new Fraction(t_.getT() * n_.getN(), t_.getN() * n_.getT()).eval2()
        }
    }
...
Fraction.fromRandom = () => {
    ...
    const d1 = random(2)
    const d2 = random(2)
    v1 = d1 === 0 ? Fraction.fromRandom() : random(10) + 1
    v2 = d2 === 0 ? Fraction.fromRandom() : random(10) + 1
...
window.onload = e => {
    ...
    console.dir(input.toCanvas(ctx, 10, 10))
}
