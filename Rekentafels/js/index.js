...
function berekenFactoren(product) {
    ...
    while (teller<=totenmet) {
        if (product%teller==0) {
            ...
            if (factor1<=maxFactor&&factor2<=maxFactor) value.push({"factor1":factor1,"factor2":factor2})
        }
        teller++
    }
...
function toonSommetje() {
    ...
    for (i=0;i<factoren[current].factoren.length;i++) insertSommetje(sommetjestabel, factoren[current].product, i==0)
}
function submitSommetjes(el) {
    ...
    if (sommetjes.length>0) {
        for (let i=0;i<sommetjes.length;i++) result = result && submitSommetje(sommetjes[i], soms)
    }
    return result
}
function volgende() {
    if (submitSommetjes(sommetjestabel)) {
        current++
        if (current>=factoren.length) stop(but_stop)
        else toonSommetje()
    }
}
