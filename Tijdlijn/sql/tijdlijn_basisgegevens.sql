...
begin
   ...
   l_first_master := true;
   for x in ( select new_ase_id, new_prr_code, min(new_ingangsdatum) new_ingangsdatum
              from pas_basisgegevens_jn
              where new_ase_id = 140910
              group by new_ase_id, new_prr_code
   ...
   loop
      ...
      dbms_output.put_line('naam: "' || x.new_ase_id || '-' || x.new_prr_code || '",');
      dbms_output.put_line('kleur: "#' || random_hex || '",');
      dbms_output.put_line('periodes: [');

      l_first_detail := true;
      for y in ( select * from pas_basisgegevens_jn where new_ase_id = x.new_ase_id and new_prr_code = x.new_prr_code order by jn_date_time asc, new_id asc ) loop
